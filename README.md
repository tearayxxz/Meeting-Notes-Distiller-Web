...
- **Layered interface:** `TiltSurface` applies bounded, desktop-only pointer depth to compact interactive surfaces (upload, file queue, and report export), while information-heavy meeting and problem views remain stable for readability. CSS theme tokens supply Light, Dark, and Web-Slinger perspective, contrast, and decorative styling.
